...
use JuLongDeviceHttp\ParamSetting\Models\NetParameters;
use JuLongDeviceHttp\ParamSetting\Models\SetNetParametersRequest;
...
//    $clientProfile->setDevicePassword("admin");
    ...
    $req = new SetNetParametersRequest();
    $req->TimeStamp = time();
    // 需要设备开启注册
    $req->Session = 'fdjlsfjeowjfldsfa';

    $netParameters = new NetParameters();
    $netParameters->DHCPEnabled = 0;
    $netParameters->IPAddress = "128.128.20.131";
    $netParameters->SubNetMask = "255.255.255.0";
    $netParameters->Gateway = "128.128.1.1";
    $netParameters->DNS1 = "114.114.114.114";
    $netParameters->DNS2 = "8.8.8.8";

    $req->Data = $netParameters;

    // 通过client对象调用 setNetParameters 方法发起请求。注意请求方法名与请求对象是对应的
    // 返回的resp是一个 SetNetParametersResponse 类的实例，与请求对象对应
    $resp = $client->setNetParameters($req);
