...
//    $clientProfile->setDevicePassword("admin");
    ...
    $req = new SetTimeParametersRequest();
    $req->TimeStamp = time();
    // 需要设备开启注册
    $req->Session = 'fdjlsfjeowjfldsfa';

    $timeParameters = new TimeParameters();
    $timeParameters->LocalTime = date("yyyy-mm-dd hh:mm:ss", time());
    $timeParameters->TimeZone = 31;
    $timeParameters->NTPEnabled = 1;
    $timeParameters->NTPServer = "ntp1.aliyun.com";
    $timeParameters->NTPPort = 123;
    $timeParameters->NTPInterval = 60;

    $req->Data = $timeParameters;

    // 通过client对象调用 SetTimeParameters 方法发起请求。注意请求方法名与请求对象是对应的
    // 返回的resp是一个 SetTimeParametersResponse 类的实例，与请求对象对应
    $resp = $client->setTimeParameters($req);
