...
use JuLongDeviceHttp\ParamSetting\Models\AudioParameters;
use JuLongDeviceHttp\ParamSetting\Models\SetAudioParametersRequest;
...
//    $clientProfile->setDevicePassword("admin");
    ...
    $req = new SetAudioParametersRequest();
    $req->TimeStamp = time();
    // 需要设备开启注册
    $req->Session = 'fdjlsfjeowjfldsfa';

    $audioParameters = new AudioParameters();
    $audioParameters->Enabled = 1;
    $audioParameters->AudioInput = 1;
    $audioParameters->AudioFormat = 3;
    $audioParameters->InputVolume = 15;
    $audioParameters->OutputVolume = 15;

    $req->Data = $audioParameters;

    // 通过client对象调用 setAudioParameters 方法发起请求。注意请求方法名与请求对象是对应的
    // 返回的resp是一个 SetAudioParametersResponse 类的实例，与请求对象对应
    $resp = $client->setAudioParameters($req);
