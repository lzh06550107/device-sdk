...
//    $clientProfile->setDevicePassword("admin");
    ...
    $req = new SetMQTTParametersRequest();
    $req->TimeStamp = time();
    // 需要设备开启注册
    $req->Session = 'fdjlsfjeowjfldsfa';

    $mqttParameters = new MQTTParameters();
    $mqttParameters->Enabled = 1;
    $mqttParameters->Address = "";
    $mqttParameters->Port = 80;
    $mqttParameters->UserName = "";
    $mqttParameters->Password = "";
    $mqttParameters->Topic = "";
    $mqttParameters->HeartbeatInterval = 60;
    $mqttParameters->ResendEnabled = 1;

    $req->Data = $mqttParameters;

    // 通过client对象调用 setMQTTParameters 方法发起请求。注意请求方法名与请求对象是对应的
    // 返回的resp是一个 SetMQTTParametersResponse 类的实例，与请求对象对应
    $resp = $client->setMQTTParameters($req);
