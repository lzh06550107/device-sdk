...
use JuLongDeviceHttp\ParamSetting\Models\FaceAlarmParameters;
use JuLongDeviceHttp\ParamSetting\Models\SetFaceAlarmParametersRequest;
...
//    $clientProfile->setDevicePassword("admin");
    ...
    $req = new SetFaceAlarmParametersRequest();
    $req->TimeStamp = time();
    // 需要设备开启注册
    $req->Session = 'fdjlsfjeowjfldsfa';

    $FaceAlarmParameters = new FaceAlarmParameters();
    $FaceAlarmParameters->Similarity = 50;
    $FaceAlarmParameters->IDCardSimilarity = 55;
    $FaceAlarmParameters->VerifyMode = 4;
    $FaceAlarmParameters->MaskCheck = 3;

    $req->Data = $FaceAlarmParameters;

    // 通过client对象调用 getFaceAlarmParameters 方法发起请求。注意请求方法名与请求对象是对应的
    // 返回的resp是一个 getFaceAlarmParametersResponse 类的实例，与请求对象对应
    $resp = $client->setFaceAlarmParameters($req);
