...
use JuLongDeviceHttp\ParamSetting\Models\GetTimeParametersRequest;
...
//    $clientProfile->setDevicePassword("admin");
    ...
    $req = new GetTimeParametersRequest();
    $req->TimeStamp = time();
    // 需要设备开启注册
    $req->Session = 'fdjlsfjeowjfldsfa';


    // 通过client对象调用 GetTimeParameters 方法发起请求。注意请求方法名与请求对象是对应的
    // 返回的resp是一个 GetTimeParametersResponse 类的实例，与请求对象对应
    $resp = $client->getTimeParameters($req);
