...
use JuLongDeviceHttp\NVRManage\Models\SearchVideoParam;
use JuLongDeviceHttp\NVRManage\Models\SearchVideoRequest;
...
//    $clientProfile->setDevicePassword("admin");
    ...
    $client = new NVRManageClient($clientProfile);

    // 实例化一个请求对象,每个接口都会对应一个request对象。
    $req = new SearchVideoRequest();
    $req->TimeStamp = time();
    // 需要设备开启注册
    $req->Session = 'fdjlsfjeowjfldsfa';

    $searchVideoParam = new SearchVideoParam();
    $searchVideoParam->StartTime = "2022-01-01 00:00:00";
    $searchVideoParam->EndTime = "2022-12-31 23:59:59";
    $searchVideoParam->PageSize = 100;
    $searchVideoParam->SimilarityThreshold = 50.00;
    $searchVideoParam->ChannelNo = 1;
    $searchVideoParam->SearchType = 2;
    $searchVideoParam->FacePicture = "";

    $req->Data = $searchVideoParam;

    // 通过client对象调用 searchVideo 方法发起请求。注意请求方法名与请求对象是对应的
    // 返回的resp是一个 SearchVideoResponse 类的实例，与请求对象对应
    $resp = $client->searchVideo($req);
