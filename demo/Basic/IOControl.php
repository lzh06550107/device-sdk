...
use JuLongDeviceHttp\Basic\Models\IOControl;
use JuLongDeviceHttp\Basic\Models\IOControlRequest;
...
//    $clientProfile->setDevicePassword("admin");
    ...
    $req = new IOControlRequest();
    $req->TimeStamp = time();
    // 需要设备开启注册
    $req->Session = 'fdjlsfjeowjfldsfa';

    $iOControl = new IOControl();
    $iOControl->ContinueSeconds = 2;

    $req->Data = $iOControl;

    // 通过client对象调用 IOControl 方法发起请求。注意请求方法名与请求对象是对应的
    // 返回的resp是一个 IOControlResponse 类的实例，与请求对象对应
    $resp = $client->IOControl($req);
