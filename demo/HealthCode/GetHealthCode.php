...
//    $clientProfile->setDevicePassword("admin");
    ...
    $client = new HealthCodeClient($clientProfile);

    // 实例化一个请求对象,每个接口都会对应一个request对象。
    $req = new GetHealthCodeRequest();
    $req->TimeStamp = time();
    // 需要设备开启注册
    $req->Session = 'fdjlsfjeowjfldsfa';

    // 通过client对象调用 getHealthCode 方法发起请求。注意请求方法名与请求对象是对应的
    // 返回的resp是一个 GetHealthCodeResponse 类的实例，与请求对象对应
    $resp = $client->getHealthCode($req);
