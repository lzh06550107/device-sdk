...
use JuLongDeviceHttp\AccessStrategy\Models\IdentifyStrategys;
use JuLongDeviceHttp\AccessStrategy\Models\SetIdentifyStrategysRequest;
...
//    $clientProfile->setDevicePassword("admin");
    ...
    $req = new SetIdentifyStrategysRequest();
    $req->TimeStamp = time();
    // 需要设备开启注册
    $req->Session = 'fdjlsfjeowjfldsfa';

    $identifyStrategys = new IdentifyStrategys();
    $identifyStrategys->PersonIdentity = PersonIdentity::$TEACHER;
    $identifyStrategys->IdentityName = PersonIdentity::getPersonGroupNameById(PersonIdentity::$TEACHER);
    $identifyStrategys->StrategyTable = [1];

    $req->List = [$identifyStrategys];
